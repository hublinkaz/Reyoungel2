...
@section('content')
    <div class="container-fluid">
        @if (\Illuminate\Support\Facades\Auth::user()->role_id==1 or \Illuminate\Support\Facades\Auth::user()->role_id ==3)
            <a class="btn btn-outline-secondary-2x" href="/panel/order/driver/{{$id}}">{{ trans('lang.Driver Assignment') }}</a>
                                ...
                                @foreach ($products as $product)
                                    ...
                                    @else
                                        <tr>
                                            ...
                                            <td class="font-success">{{ $product->order_detail_created }}</td>
                                            <td class="font-success">
                                                <form action="{{ route('manager.payment.store',$product->order_details_id) }}" method="POST">
                                                    @csrf

                                                    <input  type="text"  name="paid" id="paid" min="1" max="{{ $product->price-$product->paid }}" value="{{ $product->price-$product->paid }}" >
                                                    <button type="submit" class="form-control btn btn-success btn-xs"  data-original-title="btn btn-success btn-xs">{{ trans('lang.Add Payment') }}</button>

                                                </form>
                                            </td>

                                            <td>
                                                <form action="{{ route('manager.return.post',$product->order_details_id) }}" method="POST">
                                                    @csrf
                                                    @if($product->is_return===0)
                                                        <select style="color:#1100FF" required id="is_return" name="is_return" class="form-control"  title="Select Manager...">
