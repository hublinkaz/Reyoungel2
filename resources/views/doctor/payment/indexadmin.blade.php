						...
						<table class="display" id="export-button">
                            ...
                            @foreach ($payments as $payment)
                                 <tr >
                                        ...
                                        @if($payment->doctor_paid != $payment->manager_paid)
                                        <td style="background-color:#FF0000;color: #ffffff">
                                            <form action="{{ route('payment.status',$payment->od_id) }}" method="POST">
                                            @csrf
                                                <input  class="form-control" type="text"  name="manager_paid" id="manager_paid" value="{{ $payment->doctor_paid-$payment->manager_paid }}" >
                                                <button type="submit" class="form-control btn btn-success btn-xs"  data-original-title="btn btn-success btn-xs">Ödəniş Et</button>
                                            </form>
                                        </td>
                                        @else
